    ...
    <filter-table
      ...
      <el-table-column label="操作" width="260px">
        <template slot-scope="scope">
          <el-button type="primary" plain @click="dependenceDetail(scope.row)">依赖接口</el-button>
        </template>
      </el-table-column>
    </filter-table>
  </div>
...
export default {
  ...
  methods: {
    ...
    getTableList (params, formData = {}) {
      _common.queryDependenceServer(Object.assign(params, formData)).then((res) => {
    ...
    dependenceDetail ({ serverName, dependenceServerName }) {
      this.$router.push({ name: 'apm-dependence-detail', params: { serverName: serverName, dependenceServerName: dependenceServerName } })
    }
  }
}
</script>
